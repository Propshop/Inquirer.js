...
Forked from: https://github.com/mokkabonna/inquirer-autocomplete-prompt

> **Note:** _allowed options written inside square brackets (`[]`) are optional. Others are required._

Take `type`, `name`, `message`, `source`[, `default`, `validate`, `filter`, `when`, `pageSize`, `prefix`, `suffix`, `askAnswered`, `loop`, `suggestOnly`, `searchText`, `emptyText`, `hintText`] properties
...
**hintText** Is the text shown to instruct the user what they can type. defaults: `Use arrow keys or type to search` (and `, tab to autocomplete` if `suggestOnly` is `true`)
...
![Autocomplete prompt](./assets/inquirer.gif)
